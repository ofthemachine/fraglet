...
## Key Highlights
- **Breaking: fraglet.yaml mode management restructured** — modes are now defined inline via a `modes` map, replacing the previous per-file approach. Designed for "fat" containers with many execution modes.
...
Fields not specified in a mode inherit from the root config. All existing 100hellos containers need their `fraglet.yaml` updated to the new schema.
...
Every language container's `fraglet.yaml` must be updated to the new mode schema.
