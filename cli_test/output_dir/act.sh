...
# Locks in --output-dir behavior: mounts a real host directory writable at
# /output with no output= declaration needed at all — for wrapping a real
# CLI tool whose output filename is only known at runtime (an explicit
# flag it's passed, or its own default naming), unlike --output which
# requires the exact relpath declared ahead of time.
